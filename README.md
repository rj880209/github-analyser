...
### v1.1.0
- **Dependencies updated** — `streamlit` → 1.57.0, `groq` → 1.2.0, `requests` → 2.33.0
- **Security headers added** — `app.py` now injects `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, and `Content-Security-Policy` meta tags on every page load
- **File tree parsing fixed** — priority file matching in `github_client.py` now uses exact path comparison instead of a loose `startswith()` check, preventing false matches (e.g. `Makefile` no longer matches `Makefile.win`)
